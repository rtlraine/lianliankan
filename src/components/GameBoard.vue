...
function handleCanvasClick(e: MouseEvent) {
    ...
    if (row >= 0 && row < board.value.length && col >= 0 && col < board.value[0].length) {
        const block = board.value[row][col]
        if (!block.removed && !block.animating) {
            sound.play('click')
            
            if (board.value[row][col].selected) {
                sound.play('select')
            }
...
function handleGameOver(title: string, score: number, level: number) {
    gameOverTitle.value = title
    finalScore.value = score
    finalLevel.value = level
    gameOverModalVisible.value = true
    
    if (title === '游戏结束') {
        sound.play('gameover')
    } else {
        sound.playWinSequence()
    }
}
...
const lines = ref<{ points: { x: number, y: number }[], opacity: number }[]>([])

watch(() => stats.value.score, () => {
    const prevScore = stats.value.score - 10
    if (prevScore >= 0 && stats.value.score > prevScore) {
        sound.play('match')
    }
})

watch(() => stats.value.level, (newLevel, oldLevel) => {
    if (newLevel > oldLevel && oldLevel > 0) {
        sound.playLevelUpSequence()
    }
})
...
<template>
    <div class="game-container">
        ...
        <div class="game-board" ref="canvasRef" @click="handleCanvasClick">
            <canvas ref="canvasRef"></canvas>
            
            <div v-if="stats.value.isPaused && stats.value.isPlaying" class="pause-overlay">
                ⏸️ 暂停
            </div>
        </div>
